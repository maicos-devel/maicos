...
from .timeseries import dipole_trajectory
from ..lib.fourier import ft, ift
from ..lib.utils import check_compound, savetxt, zero_pad, bin_data
from ..lib.correlate import vcorrelate
...
class dielectric_spectrum_ion(dipole_trajectory):
    """
    ...
    :param output_prefix (str): Prefix for the output files.
    :param sol_selection (str): MDAnalysis selection command for solvent.
    :param ion_selection (str): MDAnalysis selection command for ions.
    :param temperature (float): Temperature (K)
    :param bpbc (bool): Do not make broken molecules whole again (only works if
                        molecule is smaller than shortest box vector
    ...
    def __init__(self,
                 atomgroup,
                 output_prefix="",
                 sol_selection="resname SOL",
                 ion_selection="not resname SOL",
                 temperature=300,
                 bpbc=True,
                 segs=1,
                 bins=200,
        ...
        # atomgroup[0] since dipole_trajectory is a multigroup analysis
        # and we only want first group
        ag_sol = atomgroup[0].select_atoms(sol_selection)
        ag_ion = atomgroup[0].select_atoms(ion_selection)

        super().__init__([ag_sol, ag_ion],
                         labels=["SOL", "ION"],
                         restypes=["NM", "SP"],
                         bpbc=bpbc,
                         **kwargs)
        ...
        if self._verbose:
            print("Solvent and ion groups:")
        for sel, gr in zip([ag_sol, ag_ion], [sol_selection, ion_selection]):
            if self._verbose:
                print("{:>15}: {:>10} atoms".format(gr, sel.n_atoms))
            if sel.n_atoms == 0:
                raise ValueError("No atoms found in '{}'."
                                 "Please adjust selection.".format(gr))
    ...
    def _configure_parser(self, parser):
        parser.add_argument('-o', dest='output_prefix')
        parser.add_argument('-sol', dest='sol_selection')
        parser.add_argument('-ion', dest='ion_selection')
        parser.add_argument('-segs', dest='segs')
        parser.add_argument('-bins', dest='bins')
        parser.add_argument('-nopbcrepair', dest='bpbc')
    ...
    def _prepare(self):
        ...
        self.nframes_seg = self.n_frames // self.segs
    ...
    def _calculate_acf_ion_water(self):
        ...
        acf = np.zeros(self.nframes_seg)
        for i in range(self.segs):
            acf += vcorrelate(trjs_M[i], trjs_J[i]) - vcorrelate(
                trjs_J[i], trjs_M[i])
    ...
    def _calculate_susc_ion(self):
        ...
        idx_negs = np.argwhere(
            corr_iw[idx_max:] < 0)  # Find negative crossing after the peak
        if len(idx_negs) == 0:
    ...
    def _conclude(self):
        self.results["freq"] = np.fft.rfftfreq(self.results["time_seg"].size,
                                               d=self.results["dt"])[1:-1]

        self.results["volume"] = 1e-3 * self.volume / (self.n_frames + 1)
        self.results["SOL_MD"] /= 10.0
        self.results["ION_J"] /= 10.0

        # Prefactor for autocorrelation functions
        self.pref = scipy.constants.elementary_charge**2 * 1e9
        self.pref /= 3 * self.results["volume"] * scipy.constants.Boltzmann * \
                     self.temperature * scipy.constants.epsilon_0
        ...
        self.results["susc"] = self.results["susc_w"] + self.results[
            "susc_iw"] + self.results["susc_i"]

        # Bin all of the data to avoid having too many points
        if self.bin:
            if self._verbose:
                print("Binning data above datapoint {} in log-spaced bins.".
                      format(self.binafter))
                print("Unbinned data consists of {} datapoints.".format(
                    len(self.results["freq"])))
            self.results["freq"] = bin_data(self.results["freq"],
                                            self.bins,
                                            after=self.binafter)
            self.results["susc_w"] = bin_data(self.results["susc_w"],
                                              self.bins,
                                              after=self.binafter)
            self.results["susc_iw"] = bin_data(self.results["susc_iw"],
                                               self.bins,
                                               after=self.binafter)
            self.results["susc_i"] = bin_data(self.results["susc_i"],
                                              self.bins,
                                              after=self.binafter)
            self.results["susc"] = bin_data(self.results["susc"],
                                            self.bins,
                                            after=self.binafter)
            self.results["cond"] = bin_data(self.results["cond"],
                                            self.bins,
                                            after=self.binafter)
        else:
            if self._verbose:
                print("Not binning data of length {}.".format(
                    len(self.results["freq"])))
    ...
    def _save_results(self):
        ...
        acfs = np.transpose((self.results["time_seg"], self.results["acf_w"],
                             self.results["acf_iw"], self.results["acf_i"]))
        savetxt(self.output_prefix + "acfs.dat",
                acfs,

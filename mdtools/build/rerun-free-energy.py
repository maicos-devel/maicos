...
parser.add_argument('-f', dest='mdp',   type=str,   default='grompp.mdp',
...
parser.add_argument('-e', dest='end',   type=int,   default=None,
                    help="Last lambda state from which the new FINAL charge is taken. If None last state from original mdp file is taken")
...
parser.add_argument('-q', dest='charge', type=float, default=None,
                    help="Final charge of original topology. <XXX> will be replaced with an adjusted charge depending on the lambda state.")
parser.add_argument('-nl', dest='nolink', action='store_false',
                    help="If set files will NOT linked into an ordered folder in a dhdl dir")
parser.add_argument('-d', dest='dir',   type=str,   default="lambda_",
                    help="Directory prefix, where trajectories are stored.")
parser.add_argument('-x', dest='prefix', type=str,   default="md",
                    help="file prefix for trajectory etc.")
parser.add_argument('-sub', dest='sub',  type=str,  default=None,
...
def main(firstarg=2, DEBUG=False):
    ...
    for l in range(end):
        ...
        if args.charge != None:
            for lfinal in range(begin, end)[::-1]:
                ...
                for t in "fep", "vdw", "bonded", "restraint", "mass", "temperature":
                    try:
                        mdp["{}-lambdas".format(t)
                            ] = mdp["{}-lambdas".format(t)][:lfinal + 1]
                    except KeyError:
                        pass
        ...
        if args.sub != None:
            utilities.submit_job(args.sub, "{}{}_{}_rerun".format(
                args.output, projectname, l), command)
        else:
        ...
        for lfinal in range(args.begin, end):
            if args.charge != None:
                ...
                for l in range(lfinal + 1):
                    os.symlink("../../lambda_{0}/{1}{2}{0}_{3}.xvg".format(
                        l, args.output, args.prefix, lfinal),
                        "{}{}{}.xvg".format(args.output, args.prefix, l))

                os.chdir("..")
            else:
                os.symlink("../lambda_{0}/{1}{2}{0}.xvg".format(lfinal, args.output, args.prefix),
                           "{}{}{}.xvg".format(args.output, args.prefix, lfinal))

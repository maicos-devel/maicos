...
parser.add_argument(
    ...
    required=True,
    default=None,
    help='Structure to build type.',
    choices=["armcnt", "hopg", "hopgcnt"])
parser.add_argument(
    '-i',
    '--index',
    type=int,
    required=True,
    default=None,
    help='number of unitcells for the CNT/HOPG.')
parser.add_argument(
    '-l',
    '--length',
    type=float,
    required=True,
    default=None,
    help='length of the structure [Ansgtroem]')
parser.add_argument(
    '-w',
    '--fillwater',
    action='store_true',
...
parser.add_argument(
    ...
    default=None,
    help=
    'The water number density per nm^3. If no input density will be estimated.')
parser.add_argument(
    '-x',
    '--gromacs',
    action='store_false',
...
parser.add_argument(
    '-o', dest='output', type=str, default='.', help='Output directory.')
...
def write_gro(file, data, xwidth, ywidth, zwidth):
    ...
    for index, line in enumerate(data):
        outline = "%5i%-5s%5s%5s%8.3f%8.3f%8.3f" % (
            1,
            "CNT",
            line[0],
            str(index + 1)[-5:],
            ...
            float(
                line[2] / 10.0
            )  # outline="%5i%-5s%5s%5i%8.3f%8.3f%8.3f" % (1,"CNT",line[0],index+1,float(line[1])/10.0,float(line[2]/10.0)\
            ,
            float(line[3] / 10.0))
        file.write(outline + "\n")
    ...
    if reservoir != 0:
        O_selection = oxygens[np.where(
            np.logical_or(
                ...
                (np.absolute(oxygens.positions[:, 2] - (reservoir + length / 2))
                 >= length / 2)))[0]]
    else:
        O_selection = oxygens[np.where(
            (((oxygens.positions[:, 0] - xshift)**2 +
              (oxygens.positions[:, 1] - yshift)**2) < radius**2),)[0]]

    selectionarray = np.sort(
        np.hstack((O_selection.atoms.indices, O_selection.atoms.indices + 1,
                   O_selection.atoms.indices + 2)))
...
def deletesolvate(nCatoms, nSOLresidues):
    ...
    selectionarray = np.sort(
        np.hstack(
            (randomoxygens, randomoxygens + 1, randomoxygens + 2))) + nCatoms
...
def main(firstarg=2, DEBUG=False):
    ...
    if args.structure == "armcnt":
        ...
        graphitecoords2, nHOPGatoms2 = graphite(2 * centercoord,
                                                2 * centercoord)[0:2]
    ...
    if args.fillwater == True:
        if args.structure != "hopg":
            ...
            if args.gromacs == True:
                print(
                    "Filling structure with water using gmx solvate...",
                    end=" ")
                sys.stdout.flush()
                ...
                if args.structure == "armcnt":
                    ...
                    if GMXnSOLresidues >= nSOLresidues:
                        ...
                        for i in range(nSOLresidues - GMXnSOLresidues):
                            u = cyzone(u, water, radius, InsertionShift)
                            print(
                                "%i of %i watermolecules placed." %
                                (i + 1 + GMXnSOLresidues, nSOLresidues),
                                end="\r")
                            sys.stdout.flush()
                            if (i + 1) % 500 == 0:
                    ...
                    nSOLresidues = gmxsolvate(radius, length, reservoir,
                                              nCatoms, xwidth / 2, ywidth / 2)
                    u = MDAnalysis.Universe('out.gro')
                    print("Done!")
                ...
                if args.structure == "armcnt":
                    ...
                    for i in range(nSOLresidues):
                        u = cyzone(u, water, radius, InsertionShift)
                        print(
                            "{} of {} watermolecules placed.".format(
                                i + 1, nSOLresidues),
                            end="\r")
                        sys.stdout.flush()
                        if (i + 1) % 500 == 0:
                    ...
                    nSOLresiduesbulk = int(
                        round(32.9 * reservoir * u.dimensions[0] *
                              u.dimensions[1] / 1000))
                    nSOLresidues = nSOLresiduesCNT + 2 * nSOLresiduesbulk
                    InsertionShift = np.array((u.dimensions[0] / 2,
                                               u.dimensions[1] / 2, 0))
                    ...
                    for i in range(nSOLresiduesbulk):
                        ...
                        print(
                            "{} of {} watermolecules placed.".format(
                                2 * (i + 1) + nSOLresiduesCNT, nSOLresidues),
                            end="\r")
                        sys.stdout.flush()
                        if (i + 1) % 250 == 0:
                            u.atoms.write('out.gro')
    ...
    with open('parameters.txt', 'w') as OUT:
        ...
        if args.structure != "hopg":
            OUT.write(
                str(centercoord) + '\t#center coordinate (x,y) (Angstrom)\n')
        OUT.write(str(xwidth) + '\t#box x-width (Angstrom)\n')
        OUT.write(str(ywidth) + '\t#box y-width (Angstrom)\n')
        if args.structure != "hopg":
            OUT.write(
                str(length + 2 * reservoir) + '\t#box z-width (Angstrom)\n')
        if args.fillwater == True:
            OUT.write(
                str(args.density) + '\t\t#waterdensity (Molecules/nm^-3)\n')

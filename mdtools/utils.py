...
def box(TargetUniverse,
    ...
    if InsertionShift == None:
        InsertionDomain = np.array(
            (TargetUniverse.dimensions[0], TargetUniverse.dimensions[1],
             (zmax - zmin)))

    TargetUniverse = MDAnalysis.Merge(TargetUniverse.atoms,
...
def cyzone(TargetUniverse,
           ProjectileUniverse,
           radius,
           InsertionShift=np.array([0, 0, 0]),
           zmin=0,
           zmax=None):
...
def submit_job(subfile_path,
               jobname,
               command,
               new_subfile_path=None,
               slurm_options=""):
    ...
    subprocess.call(
        'sbatch {} {}'.format(slurm_options, new_subfile_path), shell=True)

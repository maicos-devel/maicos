...
parser.add_argument('-o', dest='output', type=str,
                    default='com', help='Suffix for output filenames')
...
parser.add_argument('-dv',   dest='vdim',         type=int,   default=0,
                    help='dimension for velocity binning (0=X, 1=Y, 2=Z)', )
parser.add_argument('-nbins', dest='nbins', type=int,
                    default=200, help='number of bins')
parser.add_argument('-gr',  dest='groups',      type=str,   default='all',
                    help='Atoms for which to compute the velocity profile. This must contain only one moleculetype.', )
parser.add_argument('-nblock', dest='nblock', type=int, default=10,
                    help='maximum number of blocks for block averaging error estimate; 1 results in standard error')
...
def blockee(data):
    ...
    for i in range(0, int(np.log2(args.nblock)) - 1):
        ...
        numb = args.nblock // bs
        blocks = np.vstack(
            [np.mean(data[:, bs * i:bs * (i + 1)], axis=1) for i in range(numb)]).T
        ee.append([bs * args.dt * args.skipframes *
                   args.blockfreq, np.std(blocks, axis=1) / np.sqrt(numb - 1)])
    return ee


def fitfn(x, alpha, tau1, tau2, pref):
    return pref * (alpha * tau1 * (1 + tau1 / x * (np.exp(-x / tau1) - 1)) + (1 - alpha) * tau2 * (1 + tau2 / x * (np.exp(-x / tau2) - 1)))
...
def output(L, av_vel, av_vel_sq, binframes, isFinal=False):
    ...
    dv = np.sqrt(av_vel_sq[np.sum(binframes, axis=1) > minframes] / np.sum(binframes[np.sum(binframes, axis=1) >
                                                                                     minframes], axis=1) - v**2) / np.sqrt(np.sum(binframes[np.sum(binframes, axis=1) > minframes], axis=1) - 1)

    # make use of the symmetry
    symvel = (av_vel[:args.nbins // 4] - av_vel[args.nbins // 4:2 * args.nbins // 4][::-1] -
              av_vel[2 * args.nbins // 4:3 * args.nbins // 4] + av_vel[3 * args.nbins // 4:][::-1])
    symvel_sq = (av_vel_sq[:args.nbins // 4] + av_vel_sq[args.nbins // 4:2 * args.nbins // 4][::-1] +
                 av_vel_sq[2 * args.nbins // 4:3 * args.nbins // 4] + av_vel_sq[3 * args.nbins // 4:][::-1])
    symbinframes = (binframes[:args.nbins // 4] + binframes[args.nbins // 4:2 * args.nbins // 4][::-1] +
                    binframes[2 * args.nbins // 4:3 * args.nbins // 4] + binframes[3 * args.nbins // 4:][::-1])
    ...
    dvsym = np.sqrt(symvel_sq[np.sum(symbinframes, axis=1) > minframes] / np.sum(symbinframes[np.sum(symbinframes, axis=1)
                                                                                              > minframes], axis=1) - vsym**2) / np.sqrt(np.sum(symbinframes[np.sum(symbinframes, axis=1) > minframes], axis=1) - 1)

    header = "statistics over {:.1f} picoseconds".format(args.frame * args.dt)
    np.savetxt('vel_sym_' + args.output + '.dat', np.vstack((symz[np.sum(symbinframes, axis=1) > minframes], vsym, dvsym)).T,
               header=header)
    np.savetxt('vel_' + args.output + '.dat', np.vstack((z[np.sum(binframes, axis=1) > minframes], v, dv)).T,
               header=header)
    if isFinal and int(np.log2(args.nblock)) > 1:
        ...
        prefs = 2 * (av_vel_sq[np.sum(binframes, axis=1) > minframes]
                     / np.sum(binframes[np.sum(binframes, axis=1) > minframes], axis=1) - v**2) / (args.frame * args.dt * args.skipframes)  # 2 sigma^2 / T, (A16) in [1]
        ees = []
        count = 0
        params = []
        for i in range(args.nbins):
            if np.sum(binframes[i]) > minframes:
                ...
                def modfitfn(x, alpha, tau1, tau2): return fitfn(
                    x, alpha, tau1, tau2, pref)
                [alpha, tau1, tau2], pcov = curve_fit(modfitfn, ee_out[:, 0], (ee_out[:, i + 1])**2, bounds=(
                    [0, 0, 0], [1, np.inf, np.inf]), p0=[.99, .001, .01], max_nfev=1e5)
                # (A.17) in [1]
        ...
        np.savetxt('vel_' + args.output + '.dat',
                   np.vstack((z[np.sum(binframes, axis=1) > minframes], v, np.array(ees), dv)).T)
        np.savetxt('errest_' + args.output + '.dat', np.concatenate((ee_out[:, 0].reshape(len(ee_out), 1), (ee_out[:, 1:])[:, np.sum(binframes, axis=1) > minframes]), axis=1),
                   header='z ' + ' '.join(map(str, z[np.sum(binframes, axis=1) > minframes])))
        ...
        prefs = 2 * (symvel_sq[np.sum(symbinframes, axis=1) > minframes]
                     / np.sum(symbinframes[np.sum(symbinframes, axis=1) > minframes], axis=1) - vsym**2) / (args.frame * args.dt * args.skipframes)  # 2 sigma^2 / T, (A16) in [1]
        ees = []
        count = 0
        for i in range(args.nbins // 4):
            if np.sum(symbinframes[i]) > minframes:
                ...
                def modfitfn(x, alpha, tau1, tau2): return fitfn(
                    x, alpha, tau1, tau2, pref)
                [alpha, tau1, tau2], pcov = curve_fit(modfitfn, ee_out[:, 0], (ee_out[:, i + 1])**2, bounds=(
                    [0, 0, 0], [1, np.inf, np.inf]), p0=[.9, 1e3, 1e4], max_nfev=1e5)
                # (A.17) in [1]
                errest = np.sqrt(pref * (alpha * tau1 + (1 - alpha) * tau2))
                ees.append(errest)
                count += 1

        np.savetxt('vel_sym_' + args.output + '.dat', np.vstack(
            (symz[np.sum(symbinframes, axis=1) > minframes], vsym, np.array(ees))).T)
        np.savetxt('errest_sym_' + args.output + '.dat', np.concatenate((ee_out[:, 0].reshape(len(ee_out), 1), (ee_out[:, 1:])[:, np.sum(symbinframes, axis=1) > minframes]), axis=1),
                   header='z ' + ' '.join(map(str, symz[np.sum(symbinframes, axis=1) > minframes])))
        np.savetxt('errparams_sym_' + args.output +
                   '.dat', np.array(params))
...
def main(firstarg=2):
    ...
    if sol.n_atoms == 0:
        sys.exit(
            "\n Error: {} does not contain any atoms. Please adjust '-gr' selection.".format(args.groups))
    ...
    for ts in u.trajectory[args.beginframe + skipinitialframes:args.endframe:args.skipframes]:
        ...
        bins = (coms / (u.dimensions[args.dim] /
                        args.nbins)).astype(int) % args.nbins
        bincount = np.bincount(bins, minlength=args.nbins)
        with np.errstate(divide='ignore', invalid='ignore'):
            curvel = np.nan_to_num(np.histogram(bins, bins=np.arange(0, args.nbins + 1),
                                                weights=comvels)[0] / bincount)  # mean velocity in this bin, zero if empty

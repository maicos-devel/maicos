...
def output(diporder, av_box_length):
    ...
    outdata = np.vstack([
        z, diporder[:, 0] / args.frame, diporder[:, 1] / args.frame,
        diporder[:, 2] / args.frame
    ]).T
...
def main(firstarg=2, DEBUG=False):
    ...
    for args.frame, ts in enumerate(
        ...
        if args.com:
            ...
            waterCOM = np.sum(sol.atoms.positions[:, 2] *
                              sol.atoms.masses) / sol.atoms.masses.sum()
        ...
        dipoles = np.sum(
            list(chargepos[i::atomsPerMolecule]
        ...
        if args.binmethod == 'COM':
            ...
            coms = np.sum(
                masspos[i::atomsPerMolecule]
                for i in range(atomsPerMolecule)) / masses[:, np.newaxis]
            bins = ((coms[:, args.dim] % ts.dimensions[args.dim]) /
                    dz_frame).astype(int)
        elif args.binmethod == 'COC':
            abschargepos = sol.atoms.positions * \
                np.abs(sol.atoms.charges)[:, np.newaxis]
            charges = np.sum(
                np.abs(sol.atoms.charges)[i::atomsPerMolecule]
                for i in range(atomsPerMolecule))
            cocs = np.sum(
                abschargepos[i::atomsPerMolecule]
                for i in range(atomsPerMolecule)) / charges[:, np.newaxis]
            bins = ((cocs[:, args.dim] % ts.dimensions[args.dim]) /
                    dz_frame).astype(int)
        elif args.binmethod == 'OXY':
            bins = (
                (sol.atoms.positions[::3, args.dim] % ts.dimensions[args.dim]) /
                dz_frame).astype(int)

        bincount = np.bincount(bins, minlength=args.nbins)

        diporder[:, 0] += np.histogram(
            bins, bins=np.arange(args.nbins + 1), weights=np.dot(
                dipoles, unit))[0] / (A * dz_frame / 1e3)
        with np.errstate(divide='ignore', invalid='ignore'):
            diporder[:, 1] += np.nan_to_num(
                np.histogram(
                    bins,
                    bins=np.arange(args.nbins + 1),
                    weights=np.dot(
                        dipoles / np.linalg.norm(
                            dipoles, axis=1)[:, np.newaxis], unit))[0] /
                bincount)

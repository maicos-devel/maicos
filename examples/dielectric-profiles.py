...
# -----------
#
# In the following example, we will show how to calculate the dielectric
# profiles as described in :ref:`dielectric-explanations`.
#
...
# information for MAICoS to repair molecules for you during the analysis.
# Note, however, that unwrapping adds overhead to your calculations.
# Therefore, it is recommended to use a repaired trajectory if possible.
#
# In the following, we will give an example of a trajectory of water
# confined by graphene sheets simulated via GROMACS. We assume that the GROMACS
# topology is given by `graphene_water.tpr` and the trajectory is given by
# `graphene_water.xtc`. Both can be downloaded under
# :download:`topology <../../static/graphene_water/graphene_water.tpr>`
...
# %%
#
# This universe object can then be passed to the dielectric profile analysis
...
#
# According to the discussion above, we use an unwrapped trajectory and set the
...
#
# Since we included a large vacuum region in our simulation that is not of
# interest for the dielectric profile, we can set the ``refgroup`` to the group
# containing our water molecules. This will calculate the dielectric profile
# relative to the center of mass of the water in the region of interest.
...
# %%
# This creates the analysis object, but does not yet perform the analysis.
...
# One can also specify a ``step`` keyword to only analyze every ``step`` frames.
...
# apart the frames should be chosen to get a statistically uncertainty
# indicator estimate. For small trajectories such as the one in this
# example, this estimate is not very reliable and one should perform the
# analysis for longer trajectories for actual production runs.
#
...
# the ``eps_par`` and ``eps_perp`` attributes respectively. (See
# :class:`maicos.modules.dielectricplanar.DielectricPlanar` for a full list of
# attributes.)

# For this example, we plot both profiles using matplotlib.
# Note that MAICoS always centers the system at the origin or the selected
# refgroup, so here we set the limits of the x-axis to [-7, 7]. Then we can
# only show the relevant part of the output (the system has a width of 14 Å).
...
# The perpendicular component is given as the inverse of the dielectric profile,
# which is the "natural" output (see :ref:`dielectric-explanations` for more
# details). Furthermore, the bulk values expected for the SPC/E water model are
# given as reference lines.
#
# Notice that the parallel component is better converged than the perpendicular
# component which in this very short trajectory is still noisy.
# For trajectories with a duration of about 1 microsecond, the perpendicular
# component can be expected to be converged.

# %%
# Boundary Conditions
# -------------------
# (See :ref:`dielectric-explanations` for a thorough discussion of the
# boundary conditions).
...
# --------------------------------------------
# One has to be careful when using the dielectric profile analysis for systems
# with virtual sites, such as TIP4P water. The reason is that the virtual sites
# might not be included in the trajectory, but instead are only constructed by
# the MD engine during the force calculation. (For example some LAMMPS fixes)
#
# This problem can be circumvented by creating the virtual sites by hand. This
# is done by creating a transformation function that is added to the universe.
# This function is called for every frame and can be used to create the virtual
# sites. The following example shows how to do this for TIP4P/ε water from a
# LAMMPS trajectory. Here we only shift the oxygen charge along the H-O-H angle
# bisector by a distance of 0.105 Å, which is the distance between the oxygen
# charge and the virtual site in the TIP4P/ε water model.


def transform_lammps_tip4p(oxygen_index_array, distance):
    """
    Creates a transformation function where oxygen_index_array is the array of
    indices where atom.type == oxygen_type.
...
# The ``concfreq`` keyword is given in units of frames.
# For example, if ``concfreq = 100``, the analysis will output the current
# results to the data files every 100 frames.

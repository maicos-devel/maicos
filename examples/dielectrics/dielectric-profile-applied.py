...
# These are given by the following equations, first for the parallel component:
# .. math::
#    \varepsilon_{\parallel}^{-1} =
#    \frac{\epsilon_0 E_\parallel + m_\parallel -
#     m_{0, \parallel}}{\epsilon_0 E_\parallel}
#
# and for the perpendicular component:
# .. math::
#    \varepsilon_{\perp}^{-1} = \frac{D - m + m_0}{D},
#
...
# %%
# If we apply a field we introduce asymmetry in the system, so it is good
# practice to symmetrize the profiles. This way the profile will be the same
# as those determined from fluctuation-dissipation formalisms, which assume
# perfect symmetry. Because of the short trajectories, we use this only for the
# parallel profiles, which converge much faster than the perpendicular. Still,
# we provide the code to symmetrize the perpendicular profiles as well.
